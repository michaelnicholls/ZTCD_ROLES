...
define view entity Z_TCODES
  as select distinct from Z_TCD_AUTHS

    inner join            Z_ROLE_INFO on Z_ROLE_INFO.agr_name = Z_TCD_AUTHS.agr_name
    inner join tstct on tstct.tcode = Z_TCD_AUTHS.tcode and tstct.sprsl = $session.system_language
